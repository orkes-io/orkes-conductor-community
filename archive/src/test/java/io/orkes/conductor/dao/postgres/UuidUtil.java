 ...
 * less than 10,000 IDs are generated per millisecond on the same device (as identified by its MAC
 * address).
 */
public final class UuidUtil {
     ...
     * since 00:00:00.00 UTC, 10 October 1582. Each UUID on a particular machine is unique to the
     * 100NS interval until they rollover around 3400 A.D.
     *
     * <ol>
     *   <li>Digits 1-12 are the lower 48 bits of the number of 100 ns increments since the start of
     *       the UUID epoch.
     *   <li>Digit 13 is the version (with a value of 1).
     *   <li>Digits 14-16 are a sequence number that is incremented each time a UUID is generated.
     *   <li>Digit 17 is the variant (with a value of binary 10) and 10 bits of the sequence number
     *   <li>Digit 18 is final 16 bits of the sequence number.
     *   <li>Digits 19-32 represent the system the application is running on.
     * </ol>
     *
     * @return universally unique identifiers (UUID)
     */
    public static UUID getTimeBasedUuid(long offset) {

        final long time =
                (((System.currentTimeMillis() + offset) * HUNDRED_NANOS_PER_MILLI)
